    ...
    <div style={{position:"absolute",inset:0,background:T.white,display:"flex",flexDirection:"column"}}>
      <div style={{background:T.green,padding:"44px 24px 24px",textAlign:"center",position:"relative"}}>
        ...
        <div onClick={()=>setShowDrop(d=>!d)} style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:24,padding:"7px 16px",cursor:"pointer"}}>
          ...
          <span style={{color:T.lime,fontSize:10}}>{showDrop?"v":"v"}</span>
        </div>
        {showDrop && (
  ...
  return (
    <div style={{marginBottom:1,background:"#fff",borderBottom:"0.5px solid "+T.border}}>
      ...
      <div style={{position:"relative",width:"100%",paddingBottom:"75%",background:w.color,overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",top:10,left:10,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(8px)",borderRadius:20,padding:"4px 10px",display:"flex",alignItems:"center",gap:4}}>
          <span style={{fontSize:12}}>⭐</span>
          <span style={{color:"#fff",fontWeight:800,fontSize:13}}>{w.rating}</span>
          <span style={{color:"rgba(255,255,255,0.55)",fontSize:11}}>({w.reviews})</span>
        </div>
        {w.verified && (
          <div style={{position:"absolute",top:10,right:10,background:T.lime,borderRadius:20,padding:"4px 10px"}}>
            <span style={{color:T.green,fontSize:11,fontWeight:700}}>✅ Verificado</span>
          </div>
        )}
        {avail && (
          <div style={{position:"absolute",top:10,left:56,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(8px)",borderRadius:20,padding:"4px 10px",display:"flex",alignItems:"center",gap:5}}>
      ...
      {w.posts && w.posts.length > 1 && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:1.5}}>
          {w.posts.slice(0,3).map((p,i)=>(
            <div key={i} style={{position:"relative",paddingBottom:"100%",background:w.color}}>
              <div style={{position:"absolute",inset:0}}>
...
function HomeScreen({ go, setS, setWorker, setMatched, role, setRole, matches }) {
  ...
  return (
    <div style={{position:"absolute",inset:0,top:50,bottom:78,overflowY:"auto",background:viewMode==="feed"?T.white:T.gray5}}>
      <div style={{position:"sticky",top:0,background:"rgba(255,255,255,0.97)",backdropFilter:"blur(10px)",zIndex:20,borderBottom:"0.5px solid "+T.border,padding:"10px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div>
          <div style={{background:T.green,padding:"20px 16px 24px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
              {[{icon:"🔧",n:(matches||[]).length||2,l:"Activas"},{icon:"⭐",n:"4.9",l:"Rating"},{icon:"💰",n:"$47k",l:"Proximo pago"}].map((s,i)=>(
          ...
          <div style={{background:T.white,marginBottom:8,padding:"14px 16px"}}>
            <p style={{fontWeight:800,fontSize:15,margin:"0 0 12px"}}>Tu red</p>
            <div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:4}}>
              {WORKERS.slice(0,5).map(w=>(
                <div key={w.id} onClick={()=>{setWorker(w);setS("worker");}} style={{flexShrink:0,textAlign:"center",cursor:"pointer"}}>
                  <div style={{width:56,height:56,borderRadius:28,overflow:"hidden",border:"2.5px solid "+w.color,margin:"0 auto 4px"}}>
                    <Img src={w.avatar} style={{width:"100%",height:"100%"}}/>
                  </div>
                  <p style={{fontSize:11,fontWeight:600,margin:"0 0 1px"}}>{w.name.split(" ")[0]}</p>
                  <p style={{fontSize:10,color:T.gray3,margin:0}}>⭐{w.rating}</p>
                </div>
              ))}
            </div>
          </div>
...
function ExploreScreen({ setS, setWorker, setMatched, role, setRole }) {
  ...
  const nearby = WORKERS.filter(w=>w.distKm!=null && w.distKm<=radiusKm && (!activeCat||w.cat===activeCat));
  const filtered = WORKERS.filter(w=>(
    (!activeCat||w.cat===activeCat) &&
  ...
  return (
    <div style={{position:"absolute",inset:0,top:50,bottom:78,display:"flex",flexDirection:"column",background:T.white}}>
      <div style={{background:"rgba(255,255,255,0.97)",backdropFilter:"blur(10px)",zIndex:20,borderBottom:"0.5px solid "+T.border,padding:"10px 14px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,background:T.gray5,borderRadius:12,padding:"10px 14px"}}>
          <span style={{fontSize:15,color:T.gray3}}>🔍</span>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar servicio o trabajador..." style={{flex:1,background:"none",border:"none",outline:"none",fontSize:14}}/>
      ...
      <div style={{background:T.white,borderBottom:"0.5px solid "+T.border,padding:"8px 14px",display:"flex",gap:8,overflowX:"auto"}}>
        {[{id:null,label:"Todo"},...WORKERS.reduce((acc,w)=>{if(!acc.find(c=>c.id===w.cat)){acc.push({id:w.cat,label:w.cat});}return acc;},[])].map(cat=>(
          <div key={cat.id||"all"} onClick={()=>setActiveCat(activeCat===cat.id?null:cat.id)} style={{flexShrink:0,background:activeCat===cat.id?T.lime:activeCat===null&&!cat.id?T.black:T.gray5,borderRadius:20,padding:"6px 14px",cursor:"pointer"}}>
            <span style={{color:activeCat===cat.id?T.green:activeCat===null&&!cat.id?T.white:T.gray2,fontSize:12,fontWeight:600}}>{cat.label}</span>
          </div>
        ))}
      </div>
...
function ConfigureScreen({ matched, setS }) {
  ...
  const urgencia = modo === "ahora" ? Math.round(subtotal * 0.30) : 0;
  const gross = subtotal + urgencia;
  ...
  const canNext = step === 0 ? (modo === "ahora" || (selectedDay && selectedTime)) : true;
  ...
  return (
    <div style={{position:"absolute",inset:0,top:50,bottom:0,display:"flex",flexDirection:"column",background:T.white}}>
      <div style={{borderBottom:"0.5px solid "+T.border,padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}>
        <button onClick={()=>step===0?setS("worker"):setStep(s=>s-1)} style={{background:"none",border:"none",fontSize:20,cursor:"pointer"}}>{"<"} Atras</button>
        <span style={{fontSize:16,fontWeight:700}}>Configurar chamba ({step+1}/3)</span>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"16px"}}>
        {step === 0 && (
          <div>
            <p style={{fontWeight:800,fontSize:18,margin:"0 0 16px"}}>Cuando lo necesitas?</p>
            <div style={{display:"flex",gap:10,marginBottom:20}}>
              {[{k:"ahora",icon:"⚡",label:"Ahora (+30%)"},{k:"programar",icon:"📅",label:"Programar"}].map(m=>(
                <div key={m.k} onClick={()=>setModo(m.k)} style={{flex:1,border:"2px solid "+(modo===m.k?T.lime:T.border),borderRadius:14,padding:"16px",textAlign:"center",cursor:"pointer",background:modo===m.k?T.limeL:T.white}}>
                  <div style={{fontSize:28,marginBottom:8}}>{m.icon}</div>
                  <div style={{fontWeight:700,fontSize:14,color:modo===m.k?T.green:T.dark}}>{m.label}</div>
                </div>
              ))}
            </div>
            {modo === "programar" && (
              <div>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
                  {DAYS.map(d=>(
                    <div key={d} onClick={()=>setSelectedDay(d)} style={{background:selectedDay===d?T.lime:T.gray5,borderRadius:10,padding:"8px 14px",cursor:"pointer"}}>
                      <span style={{fontWeight:700,fontSize:13,color:selectedDay===d?T.green:T.dark}}>{d}</span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {TIMES.map(t=>(
                    <div key={t} onClick={()=>setSelectedTime(t)} style={{background:selectedTime===t?T.lime:T.gray5,borderRadius:10,padding:"8px 14px",cursor:"pointer"}}>
        ...
        {step === 1 && (
          <div>
            <p style={{fontWeight:800,fontSize:18,margin:"0 0 16px"}}>Detalles del servicio</p>
            {w && w.mobilFee > 0 && (
              <div style={{background:T.gray5,borderRadius:14,padding:"14px",marginBottom:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <p style={{fontWeight:700,fontSize:14,margin:"0 0 4px"}}>🚗 Tarifa de movilizacion</p>
                    <p style={{color:T.gray3,fontSize:12,margin:0}}>El trabajador viene a ti</p>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{fontWeight:700,color:T.limeD}}>{fmt(w.mobilFee)}</span>
                    <div onClick={()=>setMobilAccepted(b=>!b)} style={{width:44,height:26,borderRadius:13,background:mobilAccepted?T.lime:T.gray4,cursor:"pointer",position:"relative",transition:"all 0.2s"}}>
                      <div style={{position:"absolute",top:3,left:mobilAccepted?22:3,width:20,height:20,borderRadius:10,background:T.white,boxShadow:"0 1px 4px rgba(0,0,0,0.2)",transition:"all 0.2s"}}/>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {w && w.extras && w.extras.length > 0 && (
              <div>
                <p style={{fontWeight:700,fontSize:14,margin:"0 0 10px"}}>Extras opcionales</p>
                {w.extras.map(e=>(
                  <div key={e.id} onClick={()=>setSelectedExtras(p=>({...p,[e.id]:!p[e.id]}))} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px",border:"1.5px solid "+(selectedExtras[e.id]?T.lime:T.border),borderRadius:12,marginBottom:8,cursor:"pointer",background:selectedExtras[e.id]?T.limeL:T.white}}>
                    <span style={{fontWeight:600,fontSize:14}}>{e.name}</span>
                    <span style={{fontWeight:700,color:T.limeD}}>{fmt(e.price)}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {step === 2 && (
          <div>
            <p style={{fontWeight:800,fontSize:18,margin:"0 0 16px"}}>Resumen y pago</p>
            <div style={{background:T.white,border:"1px solid "+T.border,borderRadius:16,padding:"14px",marginBottom:20}}>
              {[
                {l:"Servicio base",v:fmt(base)},
                mobilFee > 0 ? {l:"Movilizacion",v:fmt(mobilFee)} : null,
                extTotal > 0 ? {l:"Extras",v:fmt(extTotal)} : null,
                urgencia > 0 ? {l:"Urgencia +30%",v:fmt(urgencia),color:"#D97706"} : null,
                {l:"Comision Chamba 10%",v:fmt(chambaFee),gray:true},
              ].filter(Boolean).map((row,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontSize:13,color:row.gray?T.gray3:T.dark}}>{row.l}</span>
                  <span style={{fontSize:13,fontWeight:600,color:row.color||T.dark}}>{row.v}</span>
                </div>
              ))}
              <div style={{borderTop:"2px solid "+T.black,paddingTop:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontWeight:800,fontSize:15}}>Total</span>
                <span style={{fontWeight:900,fontSize:22,color:T.limeD}}>{fmt(total)}</span>
              </div>
              <div style={{background:T.limeL,borderRadius:10,padding:"8px 12px",marginTop:10,display:"flex",gap:8}}>
                <span>🔒</span>
                <span style={{fontSize:12,color:T.green}}>Retenido en escrow hasta confirmar</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div style={{padding:"12px 16px",borderTop:"0.5px solid "+T.border}}>
        <button onClick={()=>{ if(step < 2) setStep(s=>s+1); else setS("payment"); }} disabled={!canNext} style={{width:"100%",background:canNext?T.lime:T.gray5,border:"none",borderRadius:14,padding:"16px",fontSize:16,fontWeight:900,color:canNext?T.green:T.gray3,cursor:"pointer"}}>
          {step < 2 ? "Siguiente ->" : "Ir al pago ->"}
        </button>
      </div>
    </div>
  );
...
function ConfirmScreen({ matched, setS, setMatches }) {
  ...
  if (stage === "rating_worker") {
    return (
      <div style={{position:"absolute",inset:0,top:50,bottom:0,overflowY:"auto",background:T.white}}>
        <div style={{background:"linear-gradient(180deg,"+T.limeL+",white)",padding:"32px 24px 20px",textAlign:"center"}}>
          <div style={{width:72,height:72,borderRadius:36,overflow:"hidden",border:"3px solid "+T.lime,margin:"0 auto 14px"}}>
            <Img src={JULIA.avatar} style={{width:"100%",height:"100%"}}/>
          </div>
          <h2 style={{fontSize:22,fontWeight:900,margin:"0 0 6px"}}>{workerName} te califica a ti</h2>
          <p style={{color:T.gray2,fontSize:13,margin:0}}>Perspectiva del trabajador</p>
        </div>
        <div style={{padding:"20px"}}>
          <div style={{background:T.gray5,borderRadius:16,padding:"16px",marginBottom:16}}>
            <p style={{fontWeight:700,fontSize:14,margin:"0 0 12px"}}>Como empleador, {workerName} dice:</p>
            <div style={{display:"flex",justifyContent:"center",gap:10,marginBottom:10}}>
              {[1,2,3,4,5].map(s=>(
                <span key={s} onClick={()=>setWorkerRating(s)} style={{fontSize:36,cursor:"pointer",opacity:workerRating>=s?1:0.25,transition:"all 0.1s"}}>⭐</span>
              ))}
            </div>
            {workerRating > 0 && <p style={{textAlign:"center",fontSize:13,color:T.green,fontWeight:600}}>{["","Dificil","Normal","Bien","Muy bien","Excelente empleador"][workerRating]}</p>}
          </div>
          <div style={{background:"#EFF6FF",borderRadius:12,padding:"12px 14px",marginBottom:20,display:"flex",gap:10}}>
            <span style={{fontSize:18}}>💡</span>
            <div>
              <p style={{fontWeight:700,fontSize:13,color:"#1D4ED8",margin:"0 0 3px"}}>Por que calificamos ambos?</p>
              <p style={{fontSize:12,color:"#1D4ED8",margin:0,lineHeight:1.5}}>Los empleadores con buenas calificaciones atraen a los mejores trabajadores.</p>
            </div>
          </div>
          <button onClick={()=>setStage("done")} disabled={workerRating===0} style={{width:"100%",background:workerRating>0?T.lime:T.gray5,border:"none",borderRadius:14,padding:"16px",fontSize:15,fontWeight:800,color:workerRating>0?T.green:T.gray3,cursor:workerRating>0?"pointer":"not-allowed",marginBottom:10}}>
            Finalizar ->
          </button>
          <button onClick={()=>setStage("done")} style={{width:"100%",background:"none",border:"none",color:T.gray3,fontSize:13,cursor:"pointer",padding:"8px"}}>Omitir</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{borderBottom:"0.5px solid "+T.lime}}>
      <div onClick={()=>setOpen(o=>!o)} style={{background:T.limeL,padding:"10px 16px",display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
        ...
        <span style={{color:T.limeD,fontSize:13}}>{open?"v":">"}</span>
      </div>
      {open && (
...
function AgendaScreen({ go, role }) {
  ...
  const UPCOMING = [
    {id:1,worker:"Ana Martinez",service:"Peluquera",date:"Mar 28",time:"10:00",price:15000,avatar:"https://i.pravatar.cc/150?img=47",color:"#BE185D",status:"confirmado"},
  ...
  const STS = {confirmado:{bg:T.limeL,c:T.limeD},pendiente:{bg:"#FEF3C7",c:"#92400E"}};
  return (
    <div style={{position:"absolute",inset:0,top:50,bottom:78,overflowY:"auto",background:T.gray5}}>
      <div style={{background:T.white,padding:"14px 16px",marginBottom:8}}>
        <p style={{fontWeight:800,fontSize:16,margin:"0 0 14px"}}>Mayo 2026</p>
        <div style={{display:"flex",gap:6}}>
          {DAYS.map((d,i)=>(
            <div key={i} onClick={()=>setSelDay(i)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4,cursor:"pointer"}}>
              <span style={{fontSize:10,color:T.gray3,fontWeight:600}}>{d}</span>
              <div style={{width:32,height:32,borderRadius:16,background:selDay===i?T.lime:"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>
                <span style={{fontSize:13,fontWeight:700,color:selDay===i?T.green:T.dark}}>{25+i}</span>
              </div>
              {[1,3].includes(i) && <div style={{width:5,height:5,borderRadius:3,background:selDay===i?T.white:T.lime}}/>}
            </div>
          ))}
        </div>
      </div>
      <div style={{background:T.white,padding:"14px 16px",marginBottom:8}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <span style={{fontWeight:800,fontSize:15}}>Proximas chambas</span>
          <span style={{color:T.gray3,fontSize:12}}>{UPCOMING.length} programadas</span>
        </div>
        {UPCOMING.map((item,i)=>(
          <div key={item.id} style={{display:"flex",gap:12,alignItems:"center",padding:"12px 0",borderBottom:UPCOMING.length-1>i?"0.5px solid "+T.border:"none"}}>
            <div style={{width:46,height:46,borderRadius:23,overflow:"hidden",border:"2px solid "+item.color+"44",flexShrink:0}}>
              <Img src={item.avatar} style={{width:"100%",height:"100%"}}/>
            </div>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between"}}>
                <span style={{fontWeight:700,fontSize:14}}>{item.worker}</span>
                <span style={{fontWeight:800,color:T.limeD,fontSize:13}}>{fmt(item.price)}</span>
              </div>
              <span style={{color:T.gray3,fontSize:12}}>{item.service}</span>
              <div style={{display:"flex",gap:8,marginTop:4}}>
                <span style={{background:STS[item.status].bg,color:STS[item.status].c,fontSize:10,fontWeight:700,borderRadius:10,padding:"2px 8px"}}>{item.status}</span>
                <span style={{color:T.gray3,fontSize:11}}>📅 {item.date} - {item.time}</span>
              </div>
            </div>
          </div>
        ))}
...
function EmployerProfile({ setS, setRole, matches }) {
  const HIRED = [
    {id:1,worker:"Ana Martinez",service:"Peluquera",date:"Vie 24 Mayo",price:15000,rating:5,avatar:"https://i.pravatar.cc/150?img=47",color:"#BE185D"},
  ...
  return (
    <div style={{position:"absolute",inset:0,top:50,bottom:78,overflowY:"auto",background:T.gray5}}>
      <div style={{background:T.white,borderBottom:"0.5px solid "+T.border,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontSize:16,fontWeight:700}}>juliamorales</span>
        <button onClick={()=>setRole("worker")} style={{background:T.limeL,border:"none",borderRadius:20,padding:"6px 14px",color:T.limeD,fontSize:12,fontWeight:700,cursor:"pointer"}}>
          Modo Trabajadora
        </button>
      </div>
      <div style={{background:T.white,marginBottom:8}}>
        <div style={{height:80,background:"linear-gradient(135deg,"+T.green+",#2D5A00)"}}/>
        <div style={{padding:"0 16px 16px",marginTop:-28}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:10}}>
            <div style={{width:60,height:60,borderRadius:30,overflow:"hidden",border:"3px solid "+T.white,background:T.gray4}}>
              <Img src={JULIA.avatar} style={{width:"100%",height:"100%"}}/>
            </div>
            <button style={{background:T.gray5,border:"none",borderRadius:10,padding:"7px 14px",fontSize:12,fontWeight:700,cursor:"pointer"}}>Editar perfil</button>
          </div>
          <p style={{fontSize:18,fontWeight:800,margin:"0 0 2px"}}>Julia Morales</p>
          <p style={{color:T.gray3,fontSize:13,margin:"0 0 12px"}}>Providencia - @juliamorales</p>
          <div style={{display:"flex",gap:0,borderTop:"0.5px solid "+T.border,paddingTop:12}}>
            {[{v:(matches||[]).length||4,l:"chambas"},{v:"892",l:"seguidores"},{v:"145",l:"siguiendo"}].map((s,i,arr)=>(
      ...
      <div style={{background:T.white,marginBottom:8,padding:"14px 16px"}}>
        ...
        {HIRED.map((h,i)=>(
          <div key={h.id} style={{display:"flex",gap:12,alignItems:"center",padding:"10px 0",borderBottom:HIRED.length-1>i?"0.5px solid "+T.border:"none"}}>
            <div style={{width:46,height:46,borderRadius:23,overflow:"hidden",border:"2px solid "+h.color+"22",flexShrink:0}}>
      ...
      <div style={{background:T.white,padding:"14px 16px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[{icon:"💰",label:"Gastado",value:"$107.000",bg:T.limeL,color:T.green},{icon:"🧹",label:"Chambas",value:"3 trabajos",bg:T.gray5,color:T.black},{icon:"⭐",label:"Rating dado",value:"5.0 / 5.0",bg:"#FEF3C7",color:"#92400E"},{icon:"🔒",label:"Protegido",value:"100% escrow",bg:"#EFF6FF",color:"#1D4ED8"}].map((s,i)=>(
  ...
  return (
    <div style={{position:"absolute",inset:0,top:50,bottom:78,overflowY:"auto",background:T.gray5}}>
      <div style={{background:T.white,borderBottom:"0.5px solid "+T.border,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontSize:16,fontWeight:700}}>juliamorales</span>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setRole("employer")} style={{background:T.dark,border:"none",borderRadius:20,padding:"6px 14px",color:T.white,fontSize:12,fontWeight:700,cursor:"pointer"}}>Modo Empleador</button>
      ...
      <div style={{background:T.white,marginBottom:8,padding:"14px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <span style={{fontWeight:800,fontSize:15}}>Mis servicios (5)</span>
          <button onClick={()=>setS("addservice","servicedetail")} style={{background:T.lime,border:"none",borderRadius:20,padding:"6px 14px",fontSize:12,fontWeight:700,color:T.green,cursor:"pointer"}}>+ Agregar</button>
        </div>
        {JULIA.services.map((svc,i)=>(
          <div key={svc.id} onClick={()=>{setSelectedService(svc);setS("servicedetail");}}
            style={{display:"flex",gap:10,alignItems:"center",padding:"12px 0",borderBottom:JULIA.services.length-1>i?"0.5px solid "+T.border:"none",cursor:"pointer"}}>
            <div style={{width:44,height:44,borderRadius:12,background:T.limeL,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>{svc.icon}</div>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontWeight:700,fontSize:14}}>{svc.name}</span>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontWeight:800,color:T.limeD,fontSize:14}}>{fmt(svc.price)}</span>
                  <span style={{color:T.gray3,fontSize:18}}>{">"}</span>
                </div>
              </div>
              <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap"}}>
                <span style={{background:T.gray5,color:T.gray2,fontSize:11,borderRadius:10,padding:"2px 8px"}}> {svc.hours}h</span>
                {svc.myMaterials.length > 0 && <span style={{background:T.limeL,color:T.limeD,fontSize:11,fontWeight:600,borderRadius:10,padding:"2px 8px"}}>🧰 Mat. incluidos</span>}
                {svc.clientMaterials.length > 0 && <span style={{background:"#FEF3C7",color:"#92400E",fontSize:11,fontWeight:600,borderRadius:10,padding:"2px 8px"}}>📋 Cliente aporta</span>}
              </div>
            </div>
          </div>
        ))}
...
function VerifyScreen({ setS }) {
  return (
    <div style={{position:"absolute",inset:0,top:50,bottom:0,background:T.white,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
      <span style={{fontSize:52,marginBottom:16}}>🛡</span>
      <h2 style={{fontSize:22,fontWeight:900,margin:"0 0 8px",textAlign:"center"}}>Verificacion de identidad</h2>
      <p style={{color:T.gray3,textAlign:"center",margin:"0 0 24px",lineHeight:1.5}}>Sube tu carnet de identidad para ganar el badge de verificado y mayor confianza de los empleadores.</p>
      <button onClick={()=>setS("profile")} style={{width:"100%",background:T.lime,border:"none",borderRadius:14,padding:"16px",fontSize:15,fontWeight:800,color:T.green,cursor:"pointer",marginBottom:10}}>Comenzar verificacion</button>
      <button onClick={()=>setS("profile")} style={{width:"100%",background:"none",border:"none",color:T.gray3,fontSize:13,cursor:"pointer"}}>Mas tarde</button>
    </div>
  );
}
...
function AddServiceScreen({ setS }) {
  const [step, setStep] = useState(0);
  ...
  return (
    <div style={{position:"absolute",inset:0,top:50,bottom:0,display:"flex",flexDirection:"column",background:T.white}}>
      <div style={{borderBottom:"0.5px solid "+T.border,padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}>
        <button onClick={()=>step===0?setS("profile"):setStep(s=>s-1)} style={{background:"none",border:"none",fontSize:20,cursor:"pointer"}}>{"<"} Atras</button>
      ...
      {JOB_OFFERS.map((job)=>{
        ...
        return (
          <div key={job.id} style={{background:T.white,marginBottom:8,padding:"14px 16px"}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button style={{flex:1,background:T.lime,border:"none",borderRadius:12,padding:"11px",fontSize:14,fontWeight:700,color:T.green,cursor:"pointer"}}>Postular</button>
...
export default function App() {
  ...
  const noNav = ["splash","login","worker","configure","payment","confirm","verify","insignias","addservice","servicedetail","chat","notif","waiting"];
  ...
  return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#0A0A0A"}}>
      <Phone>
        ...
        {screen === "addservice" && <AddServiceScreen setS={setScreen}/>}
