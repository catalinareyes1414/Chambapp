...
function WaitingForWorker({ matched, setS }) {
  ...
  useEffect(()=>{
    // Simulate worker accepting after 4 seconds
    const t1 = setTimeout(()=>setAccepted(true), 4000);
    return ()=>clearTimeout(t1);
  },[]);

  useEffect(()=>{
    if (accepted || searching) return;
    const interval = setInterval(()=>{
      setTimeLeft(t=>{
        if (t <= 1) { clearInterval(interval); setSearching(true); return 0; }
        return t - 1;
      });
    }, 1000);
      ...
      <div style={{position:"absolute",inset:0,top:50,bottom:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:T.white,padding:"0 24px"}}>
        ...
        <div style={{position:"relative",zIndex:1,width:"100%",display:"flex",flexDirection:"column",alignItems:"center"}}>
          <div style={{position:"relative",marginBottom:20}}>
            <div style={{width:100,height:100,borderRadius:50,overflow:"hidden",border:"4px solid "+T.lime,boxShadow:"0 0 0 12px "+T.limeL}}>
              <Img src={w?w.avatar:""} style={{width:"100%",height:"100%"}}/>
            </div>
            <div style={{position:"absolute",bottom:0,right:0,width:34,height:34,borderRadius:17,background:T.lime,border:"3px solid "+T.white,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>✓</div>
          </div>
          <h1 style={{fontSize:38,fontWeight:900,margin:"0 0 6px",letterSpacing:-2,color:T.black}}>Match!</h1>
          <p style={{color:T.gray2,fontSize:15,textAlign:"center",margin:"0 0 6px"}}>
            <strong style={{color:T.black}}>{w?w.name:""}</strong> acepto tu propuesta
          </p>
          <div style={{background:T.lime,borderRadius:20,padding:"5px 18px",marginBottom:24}}>
            <span style={{color:T.green,fontSize:13,fontWeight:800}}>Respondio en menos de 1 minuto</span>
          </div>
          <div style={{width:"100%",background:T.gray5,borderRadius:18,padding:"16px",marginBottom:16}}>
            {[
              {icon:"🧹",l:"Servicio",v:w?w.service:""},
              {icon:"📅",l:"Fecha",v:"Sabado 25 Mayo - 09:00"},
              {icon:"💰",l:"Total acordado",v:fmt(w?w.price+Math.round(w.price*0.1):0)},
              {icon:"🔒",l:"Pago",v:"Retenido en escrow"},
            ].map((row,i,arr)=>(
              <div key={i} style={{display:"flex",gap:12,alignItems:"center",paddingBottom:arr.length-1>i?12:0,marginBottom:arr.length-1>i?12:0,borderBottom:arr.length-1>i?"0.5px solid "+T.border:"none"}}>
                ...
                <div style={{flex:1,display:"flex",justifyContent:"space-between"}}>
                  ...
                  <span style={{fontSize:13,fontWeight:700,color:T.dark}}>{row.v}</span>
                </div>
              </div>
            ))}
          </div>
          <button onClick={()=>setS("chat")}
            style={{width:"100%",background:T.black,border:"none",borderRadius:14,padding:"16px",color:T.white,fontSize:15,fontWeight:800,cursor:"pointer",marginBottom:10}}>
            💬 Hablar con {w?w.name.split(" ")[0]:""}
          </button>
          <button onClick={()=>setS("payment")}
            style={{width:"100%",background:T.lime,border:"none",borderRadius:14,padding:"16px",color:T.green,fontSize:16,fontWeight:900,cursor:"pointer"}}>
            💳 Pagar y confirmar
          </button>
        </div>
      </div>
  ...
  if (searching) {
    return (
      <div style={{position:"absolute",inset:0,top:50,bottom:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:T.white,padding:"0 24px"}}>
        ...
        <h2 style={{fontSize:22,fontWeight:900,margin:"0 0 8px",textAlign:"center"}}>Buscando otro trabajador...</h2>
        <p style={{color:T.gray2,textAlign:"center",margin:"0 0 24px",lineHeight:1.5}}>{w?w.name:""} no respondio a tiempo. Buscando alguien disponible similar.</p>
        <div style={{width:"100%",background:T.limeL,borderRadius:14,padding:"14px",marginBottom:20}}>
          {[
            {icon:"💄",name:"Maria Lopez",rating:4.8,price:14000,eta:"2 min"},
            {icon:"🧹",name:"Carmen Soto",rating:4.9,price:16000,eta:"5 min"},
          ].map((alt,i)=>(
            <div key={i} style={{display:"flex",gap:10,alignItems:"center",marginBottom:i===0?10:0}}>
              <span style={{fontSize:22}}>{alt.icon}</span>
              <div style={{flex:1}}>
                <span style={{fontWeight:700,fontSize:14}}>{alt.name}</span>
                <span style={{color:T.gray3,fontSize:12}}> - ⭐{alt.rating}</span>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontWeight:700,color:T.limeD,fontSize:13}}>{fmt(alt.price)}</div>
                <div style={{color:T.gray3,fontSize:11}}>{alt.eta}</div>
              </div>
            </div>
          ))}
        </div>
        <button onClick={()=>setS("feed")} style={{width:"100%",background:T.gray5,border:"none",borderRadius:14,padding:"14px",fontSize:14,fontWeight:600,color:T.gray2,cursor:"pointer"}}>
          Volver al inicio
        </button>
      </div>
    );
  ...
  return (
    <div style={{position:"absolute",inset:0,top:50,bottom:0,display:"flex",flexDirection:"column",background:T.white}}>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
        <div style={{width:100,height:100,borderRadius:50,overflow:"hidden",border:"3px solid "+T.lime,marginBottom:20,boxShadow:"0 0 0 8px "+T.limeL}}>
          <Img src={w?w.avatar:""} style={{width:"100%",height:"100%"}}/>
        </div>
        <h2 style={{fontSize:22,fontWeight:900,margin:"0 0 6px",textAlign:"center"}}>Propuesta enviada</h2>
        <p style={{color:T.gray2,textAlign:"center",margin:"0 0 24px",lineHeight:1.5}}>
          Esperando que <strong style={{color:T.black}}>{w?w.name:""}</strong> acepte tu propuesta
        </p>

        {/* Timer */}
        <div style={{background:T.green,borderRadius:20,padding:"20px 40px",textAlign:"center",marginBottom:24}}>
          <p style={{color:"rgba(255,255,255,0.5)",fontSize:12,margin:"0 0 4px",textTransform:"uppercase"}}>Tiempo restante</p>
          <p style={{color:T.lime,fontWeight:900,fontSize:42,margin:0,letterSpacing:-2,fontVariantNumeric:"tabular-nums"}}>{mins}:{secs}</p>
          <p style={{color:"rgba(255,255,255,0.4)",fontSize:11,margin:"4px 0 0"}}>Si no responde, buscamos otro</p>
        </div>

        <div style={{width:"100%",background:T.gray5,borderRadius:14,padding:"14px",marginBottom:16}}>
          {[
            {icon:"📋",l:"Propuesta enviada",v:"Hace un momento"},
            {icon:"👁",l:"Vista por el trabajador",v:"Pendiente"},
            {icon:"✅",l:"Confirmacion",v:"Esperando..."},
          ].map((row,i)=>(
            <div key={i} style={{display:"flex",gap:10,alignItems:"center",marginBottom:2>i?10:0}}>
              <span style={{fontSize:16}}>{row.icon}</span>
              <div style={{flex:1,display:"flex",justifyContent:"space-between"}}>
                <span style={{fontSize:13,color:T.gray2}}>{row.l}</span>
                <span style={{fontSize:12,color:T.gray3}}>{row.v}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <button onClick={()=>setS("chat")}
          style={{width:"100%",background:"none",border:"1.5px solid "+T.border,borderRadius:14,padding:"13px",color:T.gray2,fontSize:13,fontWeight:600,cursor:"pointer",marginBottom:8}}>
          💬 Enviar mensaje mientras esperas
        </button>
    ...
    <div style={{position:"absolute",inset:0,top:50,bottom:78,overflowY:"auto",background:T.gray5}}>
      ...
      <div style={{background:T.white,marginBottom:8,padding:"14px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <button onClick={()=>setS("addservice")} style={{background:T.lime,border:"none",borderRadius:20,padding:"6px 14px",fontSize:12,fontWeight:700,color:T.green,cursor:"pointer"}}>+ Agregar</button>
        </div>
      ...
      <div style={{position:"absolute",inset:0,top:50,bottom:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:T.white,padding:"0 24px"}}>
        ...
        <div style={{width:"100%",background:T.gray5,borderRadius:16,padding:"16px",marginBottom:20}}>
          {[
            ...
            <div key={i} style={{display:"flex",gap:12,alignItems:"center",marginBottom:3>i3?0:12}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"24px 20px"}}>
          ...
          <div style={{background:T.gray5,borderRadius:14,padding:"14px"}}>
            ...
            {["Nombres y apellidos completos","Numero de RUT","Fecha de nacimiento","Foto de rostro clara","Sin reflejos ni sombras"].map((tip,i)=>(
              <div key={i} style={{display:"flex",gap:8,alignItems:"center",marginBottom:4>i4?0:6}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"24px 20px"}}>
          ...
          <div style={{background:"#FEF3C7",borderRadius:14,padding:"14px",marginBottom:16}}>
            ...
            {["Buena iluminacion en tu cara","Carnet visible y legible","Sin lentes de sol ni gorros","Fondo simple y claro"].map((tip,i)=>(
              <div key={i} style={{display:"flex",gap:8,marginBottom:i>4?0:5}}>
                <span style={{fontSize:14}}>💡</span>
